...
async function getLocationRowAsJSON(location) {
  ...
  try {
    ...
    const query = `
      SELECT * FROM buslocation WHERE bus_name = $1
    `;
    ...
    if (result.rows.length === 0) {
      console.log(`Could not find bus : ${id}`);
    } else {
...
export default getLocationRowAsJSON;
